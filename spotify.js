...
async function exchangeCode(code) {
    ...
    if (!verifier) {
        throw new Error("Spotify PKCE verifier is missing. Please authenticate again.");
    }
...
export async function handleCallback() {
    ...
    if (!returnedState || returnedState !== savedState) {
        throw new Error("Spotify authorization state validation failed.");
    }
...
async function spotifyRequest(endpoint, options = {}, retryCount = 0) {
    ...
    if (response.status === 401 && retryCount === 0) {
        ...
        return spotifyRequest(endpoint, options, retryCount + 1);
    }
    /*
     * Spotify rate limit.
     *
 ...
 */
export async function searchSpotify(query, types = "track,artist,album", limit = 20) {
    const params = new URLSearchParams({
        q: query,
        type: types,
        limit: String(limit),
...
export async function getRecentlyPlayed(limit = 20) {
    ...
    return spotifyRequest(`/me/player/recently-played?${params.toString()}`);
}
